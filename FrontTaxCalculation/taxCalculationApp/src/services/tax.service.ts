...
export class TaxService {
  ...
  getYear():any
  {
    ...
    this._http.get("https://localhost:7105/api/TaxInfoes/taxinfo/assessmentyear/" + this.userId)
    .subscribe(((data:any)=>{
      // this.customerId=data;
      // console.log(this.customerId)
      console.log(data);
      this.totalYears = data;
      return  this.totalYears;

      // this.redirecttotaxcomparison();
  ...
  compare(selectedFirstYear: number, selectedSecondYear: number){
    // let newData = JSON.stringify(data.value);

    const compareTax = {
           FirstYear: selectedFirstYear,
           SecondYear: selectedSecondYear,
           UserId:100
    };

    console.log(compareTax);
    
    this._http.post("https://localhost:7105/api/TaxInfoes/getComparison",compareTax,this.httpOptions) // where and what data
    .subscribe((data:any)=>{
      // console.log("data added:", response)
      console.log("Data received:", data);
      this.detail1 = data.detailForFirstYear;
      this.detail2 = data.detailForSecondYear;
  ...
  getAllUsers()
{
  this._http.get("https://localhost:7105/api/UserDatums")
  .subscribe((data=>{
    this.allusers=data;
  }))
}
...
viewDetails(details:any)
{
  ...
  this._http.get("https://localhost:7105/api/TaxInfoes/userdetails/"+details.userId)
  .subscribe((data=>{
